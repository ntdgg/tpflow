...
Class Tpl{
		...
		if($act =='index'){
			...
			foreach($data as $k=>$v){
				   ...
				   if($v['edit']==''){
					   ...
					   $btn ="<a class='button' onclick=Tpflow.lopen('修改','".$url_edit."','550','400')> 修改</a> <a class='button' onclick=Tpflow.lopen('设计','".$url_desc."')> 设计</a> ";
				   }else{
	...
	function WfEntrustCenter($act,$data=''){
		...
		if($act =='index'){
			...
			$html = '';
			foreach($list as $k=>$v){
				   $btn ="<a class='button' onclick=Tpflow.lopen('修改','".$urls['wfapi'].'?act=dladd&id='.$v['id']."','550','400')> 修改</a> ";
				   $sq="步骤授权";
				   if($v['flow_id']==0){
					   $sq="全局授权";
				   }
