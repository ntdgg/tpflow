...
class lib
{
	...
	public static function tpflow_btn($wf_fid,$wf_type,$status,$flowinfo,$return=0)
	{
		...
		case 0:
		  ...
		  return '<span class="btn" onclick=Tpflow.lopen(\'发起工作流\',"'.$urls['wfdo'].'?act=start&wf_type='.$wf_type.'&wf_fid='.$wf_fid.'",350,250)>发起工作流</span>';
		  break;
		case 1:
			$st = 0;
			$user_name ='';
			if($flowinfo!=-1){
				...
				if($st == 1){
					if($return==1){
					 ...
					 return '<span class="btn" onclick=Tpflow.lopen(\'审核单据信息：'.$wf_fid.'\',"'.$url['url'].'")>审核('.$user_name.')</span>';
					}else{
					if($return==1){
		...
		<table class="table table-border table-bordered table-bg" style='width:98%'>
			...
			<td style='height:80px'>
				...
				<td colspan=2 class='text-c'>
						...
						<a class="button" onclick=Tpflow.lopen("Upload","{$info['tpflow_upload']}?id=upload",135,135) style="background-color: #19be6b">附件</a> 
				</td>
				</tr>
				</table>
			</td>
   ...
   <div id="box" style='height: auto;width: auto;border: 1px solid #ccc;'>
	...
	<input type="hidden" name="process_condition" id="process_condition" value='{$one['process_tos']}'>
		<table class="tables">
			...
			<tr><td>步骤尺寸</td><td><input type="text" class="smalls" name="style_width" value="{$one['style']['width']}" style='width:60px'> X {$one['style']['height']}</td></tr>
		</table>
...
<tr><td>回退方式</td><td><select name="is_back" >
	   ...
	   <tr><td>办理人员</td><td colspan='3'><select name="auto_person" id="auto_person_id" datatype="*" nullmsg="请选择办理人员或者角色！" onchange="Tpflow.onchange(this,'auto_person');">
			...
			<input type="button" value="指定人员" onclick="Tpflow.lopen('办理人','{$urls['designapi']}?act=super_user&kid=auto_sponsor&type_mode=user','400','500')">
			</td><td> 
			...
			<input type="button" value="指定角色" onclick="Tpflow.lopen('指定角色','{$urls['designapi']}?act=super_user&kid=&type_mode=role','400','500')">
			</td><td> 
